...
public class BoardDAO extends JDBConnect {
	...
	public int getTotalRecords(int product_id) {
		int totalRecords = 0;
	...
	public List<InqueryDTO> newlist(int product_id,int page, int recordsPerPage) {
		 	List<InqueryDTO> inquiries = new ArrayList<>();
		 	int start = (page - 1) * recordsPerPage;
				
				String query = "SELECT * FROM (" +
                        "  SELECT i.*, ui.name, ROW_NUMBER() OVER (ORDER BY i.inquery_id DESC) AS rnum " +
                        "  FROM INQUERY i JOIN USER_INFO ui ON ui.IDX = i.USER_ID " +
                        "  WHERE PRODUCT_ID = ? " +
                        ") " +
				...
				try {
					...
					psmt.setInt(3, start + recordsPerPage);
					rs = psmt.executeQuery();
					...
					while(rs.next()) {
						...
						dto.setSeller_content(rs.getString("seller_content"));
	...
	public List<InqueryDTO> Inquerylist(int product_id) {
		String query = " SELECT * FROM INQUERY i JOIN USER_INFO ui ON ui.IDX = i.USER_ID WHERE PRODUCT_ID ="
				+ product_id;
	...
	public int ModalWrite(String inquery_title, String inquery_content, int product_id, int user_id) {
		int result = 0;
		...
		try {
			...
			String query = "INSERT INTO INQUERY VALUES ( "
					...
					+ "),(SELECT UI.IDX  FROM USER_INFO ui WHERE UI.IDX = ?" + "),?,?,'',SYSDATE )";
	...
	public int buy(int product_id, int product_cnt, int user_id) {
		...
		try {
			...
			String query = "INSERT INTO order_info"
					...
					+ "),?,(SELECT UI.IDX  FROM USER_INFO ui WHERE UI.IDX =?" + "),'구매완료',sysdate)";
